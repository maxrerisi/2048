...
cmap = ListedColormap(['white', 'red', 'green', 'blue', 'cyan', 'magenta', 'yellow', 'black', 'orange', 'purple', 'brown', 'pink', 'lime', 'gray', 'olive', 'navy'])
# cmap = ListedColormap([(0,0,0),(50,0,0),(100,0,0),(150,0,0)])

def show_grid(array):
    if array.size == 0:
        print("Error: Empty grid passed to show_grid.")
        return
    plt.matshow(array, cmap=cmap)
    plt.grid(color='black', linestyle='-', linewidth=0.5)

    # Customize the grid lines to go through 0.5, 1.5, 2.5, etc.
    half_ticks = np.arange(0.5, array.shape[1], 1)
    plt.gca().set_xticks(half_ticks)
    plt.gca().set_yticks(half_ticks)
    plt.gca().grid(which='minor', color='black', linestyle='-', linewidth=0.5)
    plt.gca().set_xticklabels(range(array.shape[1]))
    plt.gca().set_yticklabels(range(array.shape[0]))
